...
var app = angular.module('myApp', ['ui.router', 'ngRoute', 'ngMaterial', 'ngMessages']);
...
app.controller('MainController', function ($state, $stateParams, $mdDialog, $http, $rootScope, $scope, $location, $filter, $mdToast) {
	...
	$scope.search = function () {
		...
		$scope.pageNumber = 1;
		$http.get("https://api.github.com/search/users?q=" + $scope.searchQuery + "&page=" + $scope.pageNumber + "&per_page=100")
...
app.controller('userController',function ($stateParams, $mdDialog, $http, $rootScope, $scope,$location, $filter, $mdToast) {
	$rootScope.showGoToResult = true;
	$('#myModal').modal({
		show: true,
		backdrop: 'static'
	});
	$http.get("https://api.github.com/users/"+ $stateParams.a)
	.success(function (response) {
		...
		$http.get("https://api.github.com/users/" + $rootScope.selectedUser.login + "/followers?page=" + $scope.pageNumber + "&per_page=1000")
		.success(
			function (response) {
			$('#myModal')
			.modal(
		...
		$scope.next = function () {
			...
			$http.get("https://api.github.com/users/" + $rootScope.selectedUser.login + "/followers?page=" + $scope.pageNumber + "&per_page=100")
			.then(function (response) {
				$('#myModal')
				.modal(
					'hide');
